...
class Cascader:
    ...
    def swap(self) -> 'Cascader':
        '''
        Swap the head two elements
        '''
        if self.prev is None:
            raise IndexError("Error: No more objects to swap")
    ...
    def pluck(self, offset: int) -> 'Cascader':
        '''
        Remove the element at the specified offset and push it to the head
        '''
        if offset == 0:
            return self
        target = self.jump(offset)
    ...
    def clone(self, offset: int) -> 'Cascader':
        '''
        Copy the element at the specified offset to the head
        '''
        target = self.jump(offset)
        return self.push(target.obj)
